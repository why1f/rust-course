...
- 所有的 `package` 共享同一个[输出目录](https://beatai.org/rust-course/cargo/guide/build-cache)，该目录默认的名称是 `target` ，位于工作空间根目录下
...
若某个本地依赖包是通过 [`path`](https://beatai.org/rust-course/cargo/reference/specify-deps#通过路径引入本地依赖包) 引入，且该包位于工作空间的目录中，则该包自动成为工作空间的成员。
...
与 [package.metadata](https://beatai.org/rust-course/cargo/reference/manifest#metadata) 非常类似，`workspace.metadata` 会被 `Cargo` 自动忽略，就算没有被使用也不会发出警告。
