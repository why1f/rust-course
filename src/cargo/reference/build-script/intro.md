...
下面的部分我们一起来看看构建脚本具体是如何工作的，然后在[下个章节](https://beatai.org/rust-course/cargo/reference/build-script/examples)中还提供了一些关于如何编写构建脚本的示例。

> Note: [`package.build`](https://beatai.org/rust-course/cargo/reference/manifest#build) 可以用于改变构建脚本的名称，或者直接禁用该功能
...
当 `Cargo.toml` 设置了 `links` 时， Cargo 就允许我们使用自定义库对现有的构建脚本进行覆盖。在 [Cargo 使用的配置文件](https://beatai.org/rust-course/cargo/reference/configuration)中添加以下内容：
