...
在迭代器章节中，我们曾经提到过，迭代器的[适配器](https://beatai.org/rust-course/advance/functional-programing/iterator#消费者与适配器)分为两种：消费者适配器和迭代器适配器，前者用来将一个迭代器变为指定的集合类型，往往通过`collect`实现；后者用于生成一个新的迭代器，例如上例中的`map`。
