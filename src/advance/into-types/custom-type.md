...
何为 `newtype`？简单来说，就是使用[元组结构体](https://beatai.org/rust-course/basic/compound-type/struct#元组结构体tuple-struct)的方式将已有的类型包裹起来：`struct Meters(u32);`，那么此处 `Meters` 就是一个 `newtype`。
...
在[函数](https://beatai.org/rust-course/basic/base-type/function#永不返回的函数)那章，曾经介绍过 `!` 类型：`!` 用来说明一个函数永不返回任何值，当时可能体会不深，没事，在学习了更多手法后，保证你有全新的体验：
