...
报错跟之前无二： ``` `*mut u8` cannot be sent between threads safely```, 但是有一个问题，我们无法为其直接实现`Send`特征，好在可以用[`newtype`类型](https://beatai.org/rust-course/advance/into-types/custom-type#newtype) :`struct MyBox(*mut u8);`。
