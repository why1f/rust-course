...
> async/await 的原理就算大家不理解，也不妨碍使用 `tokio` 写出能用的服务，但是如果想要更深入的用好，强烈建议认真读下本书的 [`async/await` 异步编程章节](https://beatai.org/rust-course/advance/async/intro)，你会对 Rust 的异步编程有一个全新且深刻的认识。
...
里面有个 `features = ["full"]` 可能大家会比较迷惑，当然，关于它的具体解释在本书的 [Cargo 详解专题](https://beatai.org/rust-course/cargo/intro) 有介绍，这里就简单进行说明。
